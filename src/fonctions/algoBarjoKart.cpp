...
std::map<Point, Point> AlgoBarjoKart::algoParcours(std::map<Point, vector<Point>> listeAdjacence)
{
  ...
  // for (itr = pere.begin(); itr != pere.end(); itr++)
  // {
  //   //cout << itr->first << "\t" << itr->second << "\n";

  //   if( itr->first == Point(641,104) || itr->first == Point(641,103) || itr->first == Point(642,104) || itr->first == Point(642,103)){
  //     cout << itr->first << "\t" << itr->second << "\n";
  //     exit(EXIT_FAILURE);
  //   }
  // }

  // for (int i = 0; i < pngInfo.getPixRouges().size(); i++)
  // {
  //   cout << pngInfo.getPixRouges()[i] << endl;
  // }
  // exit(EXIT_FAILURE);
  return pere;
}
